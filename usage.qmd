...
::: {.callout-warning title="Possible issue: 404 files not found"}
If your book is in the GitHub repository `my-repo` and published
with GitHub Pages, then the base url is something like
`https://username.github.io/my-repo/` (if you did not configure a [custom
domain](https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site)).
Locally the base repository is `localhost` however. To ensure H5P works when deployed *and*
when testing locally, the code in `h5p-collector.qmd` uses a heuristic based on
the `book.site-url` configuration in the `_quarto.yml` file to find
the h5p files. So make sure this configuration is set correctly!
:::
